...
$(".modifyVersion").on("click",function(){
	...
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		if(versionid == null || versionid == ""){
			...
			window.location.href="/appversionmodifys?vid="+ versionid + "&aid="+ appinfoid;
		}
	}else{
...
$(".modifyAppInfo").on("click",function(){
	...
	var status = obj.attr("status");
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		window.location.href="/list/appinfomodify/"+ obj.attr("appinfoid");
	}else{
...
var saleSwitchAjax = function(appId,obj){
	$.ajax({
		type:"PUT",
		url:"/list/sale/"+appId+"/"+obj.attr("saleSwitch"),
		dataType:"json",
...
$(".viewApp").on("click",function(){
	var obj = $(this);
	window.location.href="qureybyidKey/"+ obj.attr("appinfoid");
});
